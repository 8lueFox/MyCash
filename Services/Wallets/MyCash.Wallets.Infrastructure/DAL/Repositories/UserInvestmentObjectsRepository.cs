...
internal sealed class UserInvestmentObjectsRepository : IUserInvestmentObjectRepository
{
    ...
    {
        var uio = await _userInvestmentObjects.Include(x => x.InvestmentObjects).ThenInclude(x => x.Transactions).SingleOrDefaultAsync(x => x.InvestmentObjects.Any(y => y.Id.Value == InvestmentObjectId), cancellationToken);
